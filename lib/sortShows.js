(function () {
  document.addEventListener("DOMContentLoaded", function () {
    const container = document.getElementById("tvshows-container") || document.getElementById("largeContainer");
    if (!container) {
      console.warn("sortShows: tvshows container not found.");
      return;
    }
    ...
    const list = nodes.map(el => {
      const idMatch = (el.id || "").match(/tvshow_(\d+)$/);
      return {
        el,
    ...
    function refreshPaginationWhenReady(attemptsLeft = 12) {
      if (window.FilterPagination && typeof window.FilterPagination.init === "function") {
        try {
          ...
          console.warn("sortShows: FilterPagination init failed, retrying...", e);
        }
