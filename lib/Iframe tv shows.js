...
function createYouTubePlayerInModal(videoId, tvshowId, showName) {
  ...
  title.textContent = showName; // Set the show name

  // Add the YouTube icon and " Trailer: " text
  const youtubeIconHTML = '<i class="fa-brands fa-youtube" style="color: #d40d0d;"></i>';
  let currentText = title.textContent.replace(" Trailer: ", ""); // Remove " Trailer: " if it exists
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  trailerLinks.forEach(function(link) {
    link.addEventListener('click', function(event) {
      ...
      var url = link.href;
      var showName = link.getAttribute('data-show-name-date'); // Get the show name attribute
      ...
      if (url.includes('youtube.com/watch') || url.includes('youtu.be/')) {
        ...
        if (videoId) {
          ...
          if (tvshowDiv && tvshowDiv.id) {
            createYouTubePlayerInModal(videoId, tvshowDiv.id, showName); // Pass the show name to the function
          } else {
